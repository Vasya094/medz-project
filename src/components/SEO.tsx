...
seo?: Partial<SeoData>
	...
	const houseDescriptions = houses
		.map((house) => {
			...
			return description ? description.slice(0, Math.floor(description.length / 2)) : '';
		})
		.filter(Boolean)
		.join('. ')
	...
	<Head>
		...
		<meta name="keywords" content={`Прокат инструмента, Прокат инструмента Астрахань, Аренда инструмента, Аренда
			инструмента в Астрахани, ${houseKeywords}, Астраханская область, Волгоградская область, Калмыкия, Республика
			Дагестан`} />
		<meta name="description"
			content="Прокат строительных инструментов и техники в Астрахани. Аренда виброплит,
			 генераторов и другого оборудования для стройки и ремонта. Доступные цены и быстрая доставка по Астраханской области." />
		<meta name="keywords"
			content="Прокат виброплит, Аренда генераторов, Строительные инструменты Астрахань, Прокат техники, Аренда бетономешалок, Виброплита Астрахань, Генератор Астрахань" />
		...
		<meta property="og:image" content={shareImage.data.attributes.formats.large?.url ??
		...
		<meta name="twitter:image" content={shareImage.data.attributes.formats.large?.url ??
			shareImage.data.attributes.url} />
